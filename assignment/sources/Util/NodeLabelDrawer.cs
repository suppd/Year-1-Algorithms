...
class NodeLabelDrawer : Canvas
{
	...
	{
		SizeF size = graphics.MeasureString(Convert.ToString(pNode.id), _labelFont);
		graphics.DrawString(Convert.ToString(pNode.id), _labelFont, Brushes.Black, pNode.location.X - size.Width / 2, pNode.location.Y - size.Height / 2);
	}
